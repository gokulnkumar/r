...
import contextlib
import scapy
from scapy.all import hexdump,conf, AsyncSniffer,sniff, Ether, IP, TCP, UDP, ARP, ICMP, get_if_addr, rdpcap, wrpcap
...
class PacketSniffer(QMainWindow):
	...
	def onePackProcess(self, pack, Date=0, Time=0, MACsource=0, MACdest=0, IPsource=0, IPdest=0, Protocol=0, length=0, info=0):
		...
		if self.dbSideStatus:
			...
			#get src and dst
			if pack.haslayer(IP): 
				with open("bad_ip.txt") as f:
            				bad_ips=f.readlines()
				for x in range(len(bad_ips)):
					bad_ips[x] = bad_ips[x].strip()
				src = pack[IP].src
				if src not in self.ipsList:
					self.ipsList.append(src)
					if src in bad_ips:
            					WebsiteWarning.MainWindow()       									
				dst = pack[IP].dst
